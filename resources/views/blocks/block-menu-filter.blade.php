<section @hasfield('section_id') id="@field('section_id')" @endfield class="block-menu-filter section">
  @php
    $menus = get_terms([
  ...
  @if($menu->have_posts()) 
    ...
    @endphp
    @if(get_the_terms(get_the_ID(), 'menu_category'))
      @foreach (get_the_terms(get_the_ID(), 'menu_category') as $cat) 
        @php $termsSLug =  strtolower($cat->name); @endphp     
      @endforeach
    @else
      @php $termsSLug =  'no-category' @endphp 
    @endif
